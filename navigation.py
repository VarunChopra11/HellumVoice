# autonomous_robot_navigation.py
...
class NavigationSystem:
    ...
    def _gps_update_loop(self):
        """Background thread to update GPS position"""
        while self.running:
            try:
    ...
    def get_waypoints(self, start_location: Tuple[float, float], 
        ...
        """Get waypoints using Google Directions API"""
    ...
    def _optimize_waypoints(self, waypoints: List[Tuple[float, float]]) -> List[Tuple[float, float]]:
        """Optimize waypoints using Douglas-Peucker algorithm"""
        if len(waypoints) <= 2:
            return waypoints
        ...
        def point_line_distance(point, line_start, line_end):
            ...
            return cross / line_length if line_length != 0 else 0
    ...
    def _adjust_speed_for_turn(self, angle_diff: float) -> int:
        """Calculate appropriate speed based on turn sharpness"""
    ...
    def _handle_obstacles(self) -> bool:
        """Check for obstacles and take avoidance action"""
        ...
        if self.state.obstacles["front"] < threshold:
            ...
            self.motors.turn_right(radius=50)
            time.sleep(0.2)
            ...
            self.motors.turn_left(radius=50)
            time.sleep(0.2)
            self.motors.move_forward()
            return True
    ...
    def navigate_to_waypoint(self, waypoint: Tuple[float, float]) -> bool:
        ...
        logger.info(f"Navigating to waypoint: {waypoint}, Distance: {distance:.2f}m, Bearing: {target_bearing:.1f}°")
        ...
        while distance > self.config["waypoint_radius"]:
            ...
            logger.debug(f"Current heading: {self.state.heading:.1f}°, Target: {target_bearing:.1f}°, Diff: {angle_diff:.1f}°")
            ...
            if abs(angle_diff) > 5:
                ...
                if angle_diff > 0:
                    ...
                    self.motors.turn_right(radius=0)
                else:
                    logger.debug("Turning left")
                    self.motors.turn_left(radius=0)
                
                start_time = time.time()
                while True:
                    current_heading = self.state.heading
                    new_angle_diff = (target_bearing - current_heading + 360) % 360
                    if new_angle_diff > 180:
                        new_angle_diff -= 360
                    ...
                    if time.time() - start_time > 5:
                        logger.warning("Heading adjustment timeout")
                        break
                    
                    time.sleep(0.1)
                
                self.motors.stop()
                speed = self._adjust_speed_for_turn(new_angle_diff)
    ...
    def navigate_route(self, destination: Union[str, Tuple[float, float]]) -> bool:
        """Navigate to a destination using waypoints"""
...
class CampusTourRobot:
    """Main class for the Campus Tour Robot"""
    ...
    def announce_arrival(self, landmark_name):
        """Announce arrival at a landmark"""
    ...
    def create_tour(self, landmark_names):
        """Create a tour visiting multiple landmarks"""
        self.current_tour = []
